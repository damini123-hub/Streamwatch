...
extern "C" {
	...
	**/
	int32_t MC_NET_UTILS_API InetPtoN(int32_t af, const int8_t *src, void *dst, uint32_t *scope = NULL);
	...
	* @return		1 on success and 0 on error
	**/
	const int8_t * MC_NET_UTILS_API InetNtoP(int32_t af, const void *src, int8_t *dst, size_t size);
	...
	**/
	int32_t MC_NET_UTILS_API ParseAddressWithPort(const int8_t *pAddress, int32_t iAddressLen, sockaddr_storage *pSockaddr, int32_t iDefaultPort, void* (*lalloc)(size_t), void (*lfree)(void*));
	...
	* @return		0 on success, -1 on error
	**/
	int32_t MC_NET_UTILS_API ParseFullUrl( const int8_t *pURI, int8_t **szPath, uint16_t &uiPort, int8_t **szServerName, int8_t **pUserName, int8_t **pPassword, void* (*lalloc)(size_t), void  (*lfree)(void*) );
	...
	**/
	int MC_NET_UTILS_API isIPAddress( const int8_t *aIP );

	/**
	* @brief Convert byte array to hex string
	*
	* @param[in]	aBuffer		Source byte array
	* @param[in]	SrcLen		Count of bytes in source array
	...
	* @return		true if conversion was successful, false on error
	**/
	bool MC_NET_UTILS_API Base64Decoder( char *szSrc, int nSrcLen, unsigned char *pbDest, int *pnDestLen );
	...
	**/
	int MC_NET_UTILS_API ConvertHexStringToValue( char *aBuffer, int SrcLen, unsigned char *&aDstBuffer, int DstLen );
	...
	**/
	MCSOCKETSERRORS MC_NET_UTILS_API IsPortBusy( int32_t aPort, const int8_t *pNIC, int32_t aProto = IPPROTO_UDP );
	...
	**/
	MCCONNECTIONTYPE MC_NET_UTILS_API GetConnectionType( const int8_t *aServerIP );
	...
	* @return		None
	**/
	void MC_NET_UTILS_API DNSResolve( const int8_t *aServerName, int8_t *aResolvedName, int32_t *aResolvedNameLen, uint32_t aAddressFamily = PF_UNSPEC );
	...
	**/
	bool MC_NET_UTILS_API IsIPv6( const int8_t *aAddress );
	...
	**/
	int32_t MC_NET_UTILS_API DetectSuitableAddressFamily( const int8_t *pAddress1, const int8_t *pAddress2 );
	...
	**/
	bool MC_NET_UTILS_API IsZeroAddress( const int8_t *aAddress );
	...
	**/
    int8_t* MC_NET_UTILS_API ConcatenateURI( const int8_t *pURI, const int8_t *pAbsURI, bool bFileURI, void* (*lalloc)(size_t), void (*lfree)(void*) );

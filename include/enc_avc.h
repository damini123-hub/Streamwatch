...
struct h264_v_settings
{
  ...
  int32_t   use_b_slices;               /**<@brief This field specifies whether to use B slices. This is not allowed for 'Baseline' profile.*/
 ...
 **/
  ...
  int32_t   use_hadamard_transform;     /**<@brief  0: SAD is used, 1: SATD is used */
/** @} */
  ...
  int32_t   video_full_range;           /**<@brief  (currently) not used */
 ...
 **/
  ...
  int32_t   hrd_preview;                /**<@brief   */

  int32_t   reserved_1_1[12];
/** @} */
 ...
 **/
  ...
  int32_t   reserved_2[383];
/** @} */
 /**
 * @name Type issues
 * @{
 **/
  int32_t   video_type;                 /**<@brief  It is just an encoder preset, which is used for example in h264VideoDefaults(). */
 ...
 **/
  ...
  int32_t   frame_mbs_mode;             /**<@brief  @ref H264_FRAME_MBS_ON, H264_FRAME_MBS_OFF */
  ...
  int32_t   write_timestamps;           /**<@brief  Write picture timecode in PT SEI */
  ...
  } custom_bit_flags;                   /**<@brief  set of flags for customers needs */
 ...
 **/
  ...
  int32_t   me_subpel_mode;             /**<@brief  @ref H264_FULL_PEL, @ref H264_HALF_PEL or @ref H264_QUARTER_PEL */
  ...
  int32_t   enable_fast_intra_decisions;/**<@brief  Enables fast intra decisions */
  int32_t   enable_fast_inter_decisions;/**<@brief  Enables fast inter decisions */
 ...
 **/
  ...
  int32_t   hierar_p_frames;            /**<@brief  GOP structure with hierarchically coded P frames (for temporal scalability with out using B frames) (not yet supported in enc_avc/trunk) */

  int32_t   reserved_6[47];
/** @} */
 ...
 * @{
 **/
  int32_t   enable_intra_big;           /**<@brief  Allows to use 16x16 intra mode in intra slices */
  int32_t   enable_intra_8x8;           /**<@brief  Same for 8x8 */
  int32_t   enable_intra_4x4;           /**<@brief  Same for 4x4 */
  int32_t   enable_intra_pcm;           /**<@brief  Same for PCM */

  int32_t   enable_inter_big;           /**<@brief  Allows to use 16x16 intra mode in inter slices */
  int32_t   enable_inter_8x8;           /**<@brief  Same for 8x8 */
  int32_t   enable_inter_4x4;           /**<@brief  Same for 4x4 */
  int32_t   enable_inter_pcm;           /**<@brief  Same for PCM */
...
/** @} */
  int32_t   adaptive_quant_strength[8]; /**<@brief  Adaptive quantization strength [-100..100] or every mode (from H264_AQUANT_MODE_BRIGHTNESS to COMPLEXITY), last 5 strengths are not used at the moment */
...
/**
 * @brief Fill @ref h264_v_settings - structure with defaults values.
 ...
 */
char * MC_EXPORT_API h264OutVideoDefaults(struct h264_v_settings *set, int32_t video_type, int32_t PAL);

/**
 * @brief Fill @ref h264_v_settings - structure with defaults values according to resolution and framerate.
 * @brief Use only for Intra and LongGOP presets.
 * @param[out] set destination for encoder settings
 * @param[in] video_type encoder presets: Panasonic P2 Intra and LongG, Intra RP2027, XAVC Intra and LongGOP.
 * @param[in] PAL video standard - PAL (1) or NTSC (0)
 ...
 */
char * MC_EXPORT_API h264OutVideoDefaultsExt(struct h264_v_settings * set, int32_t video_type, int32_t PAL, int32_t width, int32_t height, double frame_rate);


/**
 * @brief Fill @ref h264_v_settings - structure with defaults values according to resolution, frame rate and interlace mode 
 * @brief Use only for Intra and LongGOP presets.
 * @param[out] set destination for encoder settings
 * @param[in] video_type encoder presets: Panasonic P2 Intra and LongG, Intra RP2027, XAVC Intra and LongGOP.
 * @param[in] PAL video standard - PAL (1) or NTSC (0)
 ...
 * @param[in] height specifies height of the encoded video
 * @param[in] frame_rate specifies the frame rate of the encoded video
 * @param[in] interlace specifies the frame type of the encoded video: 0 - progressive, 1 - interlaced, 2 - MBAFF
 */
char * MC_EXPORT_API h264OutVideoGetDefaults(struct h264_v_settings * set, int32_t video_type, int32_t PAL, int32_t width, int32_t height, double frame_rate, int32_t interlace_mode);

/**
 * @brief Change performance-related part of @ref h264_v_settings structure
 ...
 * @param[in] app
 * @return Return one of  defined errors (@ref H264ERROR_NONE , @ref H264ERROR_FAILED or @ref H264ERROR_PARAM_ADJUSTED)
 */
 ...
 * @param[in] instance the video encoder instance.
 * @return Return fullness of coded picture buffer
 */

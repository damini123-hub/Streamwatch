...
//! @brief Specifies the type of VC-3 video essence_coding
...
typedef struct mxfdmux_stream_format_s
{
  ...
  uint8_t frame_layout;                 //!< @brief Frame layout of a video stream (according to SMPTE 377M E.2.2).

  uint32_t picture_essence_coding;	    //!< @brief Picture essence coding value of a video stream. One of ESSENCE_xxx... defined above

  int64_t duration;                     //!< @brief Stream duration in edit units, -1 is unknown value. On growing file this is the number of edit units the parsed so far.
  ...
  uint64_t origin;                      //!< @brief Timeline track origin - 1st valid frame on timeline - default 0.

	uint64_t sample_count;                //!< @brief Actual number of samples in stream.

	uint8_t field_dominance;              //!< @brief Field dominance for interlace layout.

  uint32_t track_id;                    //!< @brief MXF Track ID, not identical with the 0 based stream ID above.

  int64_t positionable_duration;        //!< @brief Duration in which the stream is positionable. On growing file this can be less than 'duration' and means seeks are possible up to this range.

  int64_t positionable_sample_count;    //!< @brief Duration in samples in which the stream is positionable. On growing file this can be less than 'sample_count' and means seeks are possible up to this range.
...
struct mxfdmux_file_info
{
  ...
  uint8_t reserved_1[4];					  //!< @brief Reserved
...
//!
//! @param[in]      rc_app_ptr	Pointer to a get resource function 
...
mxfdmux_tt* MC_EXPORT_API mxfDemuxNew(void *(MC_EXPORT_API *get_rc_ex)(void *rc_app_ptr, const char* name), void *rc_app_ptr, struct mxfdmux_settings *set);
...
//! @param[in]    parser_num		Parser number
//!
//! @return  non-zero if successful, the number of bytes consumed from the source stream, 0 if EOF, -1 if an error occurred, -2 if there's not enough data but EOF was not yet detected (growing file).
...
//! The Buffer will be filled with with the KLV wrapped framework data (V), except key (K) and length (L) values, starting from the byte of the value field
//!
//! @param[in]      demuxer			      Pointer to a demuxer instance

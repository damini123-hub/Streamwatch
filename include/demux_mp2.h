...
typedef struct mp2dmux_index_info_s
{
    ...
    int64_t DTS;                                //!< depreciated, no longer used!
...
typedef struct mp2dmux_PSI_callback_settings_s
{
    ...
    mp2_PSI_callback PSI_callback;              //!< pointer to a callback routine
...
/*!
   ...
   @param [in] get_rc_ex     a pointer to a get_rc_ex resource function
   @param [in] rc_app_ptr   an application supplied pointer that will be passed back with any get_rc calls
...
*/
mp2dmux_tt * MC_EXPORT_API mp2DemuxNew(void *(MC_EXPORT_API *get_rc_ex)(void *rc_app_ptr, const char* name), void *rc_app_ptr, mp2dmux_settings_t *set);
...
/*!
    @brief Call to get state information.

    @param [in] demuxer  a pointer to a demuxer instance
    @param [in] info     a pointer to an mc_state_info_t structure

    @return              0 if successful, non-zero if unsuccessful
*/
int32_t MC_EXPORT_API mp2DemuxGetStateInfo(mp2dmux_tt *demuxer, mc_state_info_t *info);
...
/*!
    @brief Call to update the size of the source file when using external I/O mode
    (if it is getting bigger). File mode only.

    @param [in] demuxer     a pointer to a mp2dmux_tt object created with a mp2DemuxNew call
    @param [in] parser_num  a parser number from a mp2DemuxNewParser call
    @param [in] file_size   the new file size if external i/o mode, else unused

    @return                 0 if successful, non-zero if unsuccessful
*/
int32_t MC_EXPORT_API mp2DemuxUpdateFilesize(mp2dmux_tt *demuxer, int32_t parser_num, int64_t file_size);
...
*/
mpgdmux_tt * MC_EXPORT_API mpegDemuxNew(void *(MC_EXPORT_API *get_rc_ex)(void *rc_app_ptr, const char* name), void *rc_app_ptr, struct mpgdmux_settings *set);

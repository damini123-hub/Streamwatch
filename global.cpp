		...
		if(pOutput->m_bIndex_Diff == true)
		{
			...
			if((pCInput->m_bPCRPID_Found == true && bStartVideoAnalysis == false) || (bStartVideoAnalysis == true && pCInput->m_bVideoHealth == false && pCInput->m_bPCRPID_Found == true))
			{
						...
						if(pCInput->m_nPcr_TempValue != 0)
						{
							...
							{		
								...
								if(i == (pOutput->m_nSendOffset - 1))
								{
									...
									if(pOutput->m_nSendOffset < 50 || pOutput->m_nSendOffset > 75)
									{
										_stprintf(strMsgoffset, _T("Sender() - nDiffPCR = %d --- PCR_TimeDiff = %.02f"), pOutput->m_nSendOffset, pOutput->m_nTimeDiff);
										//LogDebugInfo(strMsgoffset, pStream->m_nChannel_Index);
										//OutputDebugString(strMsgoffset);
									}

									if(pOutput->m_nTimeDiff > 40 && pOutput->m_nTimeDiff < 1000)
									{
										//_stprintf(strMsgoffset, _T("**** Send Packet GAP = %d ||||| PCR_GAP = %.02f\n"), pOutput->m_nSendOffset, pOutput->m_nTimeDiff);
										_stprintf(strMsgoffset, _T("Sender() - nDiffPCR = %d --- 40 < PCR_TimeDiff < 1000 = %.02f"), pOutput->m_nSendOffset, pOutput->m_nTimeDiff);
										//LogDebugInfo(strMsgoffset, pStream->m_nChannel_Index);
										//OutputDebugString(strMsgoffset);
									}
									...
									if(pOutput->m_nTimeDiff > 1000 || pOutput->m_nTimeDiff < 0)
									{
										...
										_stprintf(strInfo, _T("Sender() - Error:  nDiffPCR = %d --- 0 > PCR_TimeDiff > 1000  = %.02f"), pOutput->m_nSendOffset, pOutput->m_nTimeDiff);
										//LogDebugInfo(strInfo, pStream->m_nChannel_Index);
									...
									if(SEND_UDP_G)
									{
										...
										if (lPrevPerformanceCount.QuadPart > 0)
										{
											QueryPerformanceCounter(&lPerformanceCount);
											WaitAccumulate = (double)(lPerformanceCount.QuadPart - lPrevPerformanceCount.QuadPart) * 1000 / lPerformanceFreq.QuadPart;
											diffWait = pOutput->m_nTimeDiff + lwaitadd - WaitAccumulate;
										}
										else
											diffWait = 0;
				...
				if(nCurrentIndex_diff > 0)
				{
					...
					TCHAR strInfo[200];
					_stprintf(strInfo, _T("Sender() - Diff = %d -- W = %d -- R = %d"), nCurrentIndex_diff, pCInput->m_nIndex_Write, pOutput->m_nIndex_Read);
					//LogDebugInfo(strInfo, pStream->m_nChannel_Index);
					//OutputDebugString(strInfo);
				}
				else
...
void LogDebugInfo(TCHAR* strMsg , int streamId)
{
	...
	try{
		...
		/*if(streamId == -1)
			...
			_stprintf_s(strFile, MAX_PATH, _T("%s\\Stream_%d_DebugInfo_%04d-%02d-%02d-%02d.log"), folderpath, streamId + 1, lt.wYear, lt.wMonth, lt.wDay, lt.wHour);		
		...
		if(bFileTooBig)
		{
			...
			/*if(streamId == -1)
				...
				_stprintf_s(strNewFileName, MAX_PATH, _T("%s\\Stream_%d_DebugInfo_%04d-%02d-%02d-%02d_Part_%02d-%02d-%03d.log"), folderpath, streamId + 1, lt.wYear, lt.wMonth, lt.wDay, lt.wHour, lt.wMinute, lt.wSecond, lt.wMilliseconds);
			try
			{
				CFile::Rename(strFile, strNewFileName);
			}
